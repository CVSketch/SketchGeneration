...
Fig. 1：本文模型在奖励中使用带 Content Masking 的裁剪 $L_1$ 损失训练；基线用对抗损失。
...
Fig. 3 展示固定笔宽、最大笔长、不透明、白底的影响：基线能复现目标，但 fidelity 低于 Huang et al.；笔更细、更短，需要更多笔；固定宽度难以画嘴、眼等细节。把画布划成子块、分块跑模型，等价于用更细的刷（Fig. 6：整图 125 笔 + 每个 1/25 子块各 125 笔）。子块策略下，基线最终损失可接近 Huang et al.
...
用式 (4) 作奖励时，模型易陷局部最优，**只输出空白画布**（参数怎么调都这样）。原因：VGG 在照片上学到的特征，对只有少量笔的画布几乎随机；Fig. 7 显示即使成画已很像目标，提取特征仍与照片目标差很远。未来可试纹理偏置更小的分类器（Geirhos et al. 2019）。

### 4.3 Content Masked 奖励与 VGG 层选择

![[图/Content-Masked-Loss/fig8.png]]

VGG-16 含 13 个卷积层与 5 个池化层，前 29 层可用于掩码（Fig. 8：浅层偏边缘纹理，深层偏眼、嘴等）。用式 (5) 在不同层训练多组模型，中间笔序不同：浅层更铺整脸，深层更盯抽象部位；**最终成画相近**，但深层掩码的中间过程更像人。后续实验固定 **第 17 层**，在高低层特征间折中。

![[图/Content-Masked-Loss/fig9.png]]

Fig. 9：不同 VGG 层生成 Content Mask 时，训练得到的中间笔序对比。
...
**AI 评估**：各模型对 hold-out 中 **2000** 张肖像逐笔作画；每加一笔用 FaceNet（Schroff, Kalenichenko, and Philbin 2015）检测是否有人脸。Fig. 11 为「在指定笔数内检测到脸的比例」。Content Masking 总体提升 **50–200 笔** 区间的检出率；对 $L_2$ 提升较弱（深色偏好可能掩盖掩码）。**CM + $L_1^*$** 检出最高。

![[图/Content-Masked-Loss/fig12.png]]

![[图/Content-Masked-Loss/fig13.png]]

**人类评估**：五模型逐笔作画；向工人展示 **10、30、100、200、750** 笔时的画布（Fig. 12 为例）。五张画来自五模型，问「哪张最像脸」。算法顺序与笔数顺序随机。**332** 名独立 MTurk 工人；**400** 张图；每题 **4** 人评。Fig. 13：在 **30–200 笔**，**CM + $L_1^*$** 被选为「最像脸」的比例明显更高；CM + $L_2$ 提升不显著，作者归因于 $L_2$ 的深色偏置——**CM 配 $L_1^*$ 更成功**。750 笔附近各模型差距缩小。
